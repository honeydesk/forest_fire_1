...
def predict_classification():
    try:
        ...
        list1 =  list(dict_test.values())
        input = [eval(i) for i in list1]
...
def batch_regression():
    try:
        ...
        db_obj = MongoDbUtils()
        if db_obj.isDatabasePresent('batch_data'):
            if db_obj.isCollectionPresent('regression_batch_data'):
                response = db_obj.getRecords('regression_batch_data')
                if response is not None:
